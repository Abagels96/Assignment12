...
INSERT INTO customers_pizzas(
  pizza_id, customer_id, quantity, order_id
) 
VALUES 
  (3, 1, 2, 1), 
  (4, 1, 1, 1), 
...
FROM 
  customers c 
  JOIN customers_pizzas cp ON c.customer_id = cp.customer_id 
  JOIN pizzas p ON cp.pizza_id = p.pizza_id 
GROUP BY 
  c.customer_id;
...
FROM 
  ...
  JOIN customers_pizzas cp ON cp.customer_id = c.customer_id 
